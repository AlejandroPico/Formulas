import { rangeControl, pluginLoop, clearCanvas, line, dot, label, colorAlpha, clamp } from "../../shared/plugin-utils.js";

export function mountBernoulliPlugin({ canvas, controls, readout }) {
  controls.innerHTML = `
    ${rangeControl("Seccion de salida A2", "a2", 20, 100, 1, 52, "%")}
    ${rangeControl("Velocidad inicial v1", "v1", 0.5, 8, 0.1, 2.0, "m/s")}
    ${rangeControl("Densidad rho", "rho", 500, 2000, 50, 1000, "kg/m3")}
  `;
  ...
  return pluginLoop(canvas, controls, readout, (ctx, w, h, values, time, palette) => {
    ...
    drawManometer(ctx, 52, cy - r1 - 18, p1, 320000, palette.accent, "P1");
    drawManometer(ctx, w - 92, cy - r2 - 18, Math.max(0, p2), 320000, palette.accent2, "P2");
    label(ctx, `v1 ${v1.toFixed(1)} m/s`, 32, cy + r1 + 28, palette);
    label(ctx, `v2 ${v2.toFixed(1)} m/s`, w - 144, cy + r2 + 28, palette);
    readout.innerHTML = `
      <strong>Bernoulli / Venturi</strong>
      <span>P1 ${(p1 / 1000).toFixed(0)} kPa; energia cinetica ${(0.5 * rho * v1 * v1 / 1000).toFixed(2)} kPa</span>
      <span>P2 ${(Math.max(0, p2) / 1000).toFixed(1)} kPa; energia cinetica ${(0.5 * rho * v2 * v2 / 1000).toFixed(2)} kPa</span>
      <em>Al estrecharse el tubo, la velocidad sube y la presion estatica baja.</em>
    `;
  });
}
