...
export function mountContinuityPlugin({ canvas, controls, readout }) {
  controls.innerHTML = `
    ${rangeControl("Area de entrada A1", "a1", 60, 160, 1, 120, "cm2")}
    ${rangeControl("Velocidad de entrada v1", "v1", 0.5, 5, 0.1, 1.6, "m/s")}
    ${rangeControl("Area de salida A2", "a2", 30, 160, 1, 72, "cm2")}
  `;
  ...
  return pluginLoop(canvas, controls, readout, (ctx, w, h, values, time, palette) => {
    ...
    drawSection(ctx, w * 0.18, cy, r1, palette.warning, "A1");
    drawSection(ctx, w * 0.82, cy, r2, palette.danger, "A2");
    drawVelocityArrow(ctx, w * 0.22, cy + r1 + 24, v1, palette.accent, "v1", palette);
    drawVelocityArrow(ctx, w * 0.72, cy + r2 + 24, v2, palette.accent2, "v2", palette);
    label(ctx, `Q = A*v = ${q.toFixed(1)}`, Math.max(18, w / 2 - 64), 32, palette);

    readout.innerHTML = `
      <strong>Conservacion de caudal</strong>
      <span>A1v1 = ${A1.toFixed(0)} * ${v1.toFixed(1)} = <b>${q.toFixed(1)}</b></span>
      <span>A2v2 = ${A2.toFixed(0)} * ${v2.toFixed(2)} = <b>${q.toFixed(1)}</b></span>
      <em>Si el area se estrecha, la velocidad aumenta para conservar el flujo de masa.</em>
    `;
  });
}
