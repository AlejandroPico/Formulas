...
export function mountEulerPlugin({ canvas, controls, readout }) {
  ...
  return pluginLoop(canvas, controls, readout, (ctx, w, h, values, time, palette) => {
    ...
    readout.innerHTML = nearPi
      ? `<strong>Identidad de Euler</strong><span>Valor especial en pi: -1</span><em>La identidad relaciona exponencial compleja, pi, i, uno y cero.</em>`
      : `<strong>Formula de Euler</strong><span>cos(x) = ${cosValue.toFixed(3)}; sin(x) = ${sinValue.toFixed(3)}</span><em>El punto recorre el circulo unitario.</em>`;
  });
}
