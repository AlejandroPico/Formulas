import { rangeControl, pluginLoop, readControls, clearCanvas, line, dot, label, arrow, attachDrag, colorAlpha, clamp } from "./plugin-utils.js";

export function mountGravityPlugin({ canvas, controls, readout }) {
  controls.innerHTML = `
    ${rangeControl("Masa 1 m₁", "m1", 10, 220, 1, 80, "kg")}
    ${rangeControl("Masa 2 m₂", "m2", 10, 220, 1, 120, "kg")}
    ${rangeControl("Escala de fuerza", "scale", 0.4, 3, 0.1, 1.2, "×")}
  `;
  ...
  function draw(ctx, w, h, values, time, palette) {
    ...
    drawBody(ctx, p1, palette.accent, "m₁", palette);
    drawBody(ctx, p2, palette.accent2, "m₂", palette);
    label(ctx, `r = ${r.toFixed(0)} px`, (p1.x + p2.x) / 2 - 42, (p1.y + p2.y) / 2 - 10, palette);
    readout.innerHTML = `
      <strong>Gravitación universal</strong>
      <span>F = G · (${m1.toFixed(0)} · ${m2.toFixed(0)}) / ${r.toFixed(0)}² = <b>${force.toFixed(4)}</b> unidades</span>
