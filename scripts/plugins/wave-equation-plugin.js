import { rangeControl, pluginLoop, clearCanvas, line, label, clamp } from "./plugin-utils.js";

export function mountWaveEquationPlugin({ canvas, controls, readout }) {
  controls.innerHTML = `
    ${rangeControl("Velocidad de onda c", "speed", 0.4, 3.8, 0.1, 1.7, "")}
    ${rangeControl("Amortiguación", "damping", 0, 0.018, 0.001, 0.004, "")}
    ${rangeControl("Tensión visual", "tension", 0.35, 1.4, 0.05, 0.85, "")}
  ...
  return pluginLoop(canvas, controls, readout, (ctx, w, h, values, time, palette) => {
    ...
    readout.innerHTML = `
      <strong>Ecuación de onda 1D</strong>
      <span>u_tt = c^2 u_xx · c=<b>${values.speed.toFixed(1)}</b></span>
      <span>energía visual=${energy.toFixed(2)}</span>
      <em>El pulso se propaga, refleja en los extremos y se amortigua suavemente para mantener la simulación estable.</em>
    `;
  });
}
...
function drawWave(ctx, w, h, state, palette) {
  ...
  label(ctx, "propagación + reflexión", Math.max(18, w - 178), 30, palette);
}
