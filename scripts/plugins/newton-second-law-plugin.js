import { rangeControl, pluginLoop, clearCanvas, line, label, arrow, clamp } from "./plugin-utils.js";
...
export function mountNewtonSecondLawPlugin({ canvas, controls, readout }) {
  ...
  return pluginLoop(canvas, controls, readout, (ctx, w, h, values, time, palette) => {
    ...
    label(ctx, `a=${acceleration.toFixed(2)} m/s²`, clamp(boxX + size / 2 - 58, 10, w - 130), Math.max(28, boxY - 26), palette);
    label(ctx, `m=${mass.toFixed(1)} kg`, 18, 32, palette);

    readout.innerHTML = `
      <strong>Dinámica en tiempo real</strong>
      <span>F = m·a → ${force.toFixed(1)} = ${mass.toFixed(1)}·<b>${acceleration.toFixed(2)}</b></span>
      <span>v visual = ${state.v.toFixed(2)}</span>
      <em>La misma fuerza acelera menos a masas grandes; cambiar el signo de F invierte la aceleración.</em>
    `;
  });
}
