...
export default function mount({root,canvas,controls,readout}){
  ...
  function drawTangents(path){ctx.save();for(let i=38;i<path.length-1;i+=44){const p=path[i],q=path[i+1],ang=Math.atan2(q.y-p.y,q.x-p.x);arrow(p.x,p.y,Math.cos(ang)*30,Math.sin(ang)*30,'#f59e0b','')}label('vectores tangentes: localmente sigue recto',38,canvas.height-34,'#fbbf24');ctx.restore()}
  ...
  function summary(g){return `<b>Γ=${g.k.toFixed(2)} · velocidad=${state.speed.toFixed(1)} · impacto=${state.impact.toFixed(0)} · desvío≈${g.deflection.toFixed(1)}°.</b><br>La línea gris es lo que esperaría una geometría plana. La curva azul es la geodésica: la partícula no gira porque “sienta una fuerza lateral”, sino porque avanzar recto localmente dentro de una geometría curvada produce una curva global.`}
